...
def main():
    ...
    print "Processing Start file:{0}, max gap:{1}, end_frames:{2}, minimum_gross_losses:{3} delimiter:'{4}'".format( filename, max_frames_gap, ignore_end_frames, minimum_gross_losses, delimiter )
    print 'start load', datetime.datetime.now()
    games = load_games_from_json( filename )
    print 'load complete', datetime.datetime.now()
    columns, bucketed_sums = ArmyRatioReport.get_report(games, max_frames_gap, ignore_end_frames, minimum_gross_losses)
